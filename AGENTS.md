...
- Entering snip mode requests best-effort Paddle preloading without waiting before selection/capture. Only preload when Paddle is selected; use the normal service initialization queue and cached sessions with current GPU settings. Route Chromium through the shared offscreen bootstrap and Firefox directly through the background service. Preload failure must not block a later scan.
