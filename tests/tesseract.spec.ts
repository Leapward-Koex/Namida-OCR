...
test('Tesseract recognizes concurrent horizontal snips offline', async ({ context, page, serviceWorker, extensionId }) => {
    ...
    const cases = ['general-001-horizontal-japanese', 'general-002-mixed-japanese-latin-digits', 'general-010-serif-japanese']
        .map((name) => ocrCases.find((entry) => entry.name === name)!);
