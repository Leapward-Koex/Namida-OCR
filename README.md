...
- `npm run prepare:paddleocr-onnx` regenerates the default `server` bundle, and `npm run prepare:paddleocr-onnx:mobile` regenerates the Firefox/mobile bundle, using [prepare-paddleocr-onnx.py](/c:/Dev/Namida/prepare-paddleocr-onnx.py) to download official PaddleOCR repos, export ONNX files, normalize unsupported `MaxPool ceil_mode` attributes for ONNX Runtime Web acceleration, and refresh the committed bundle metadata.
